...
with st.sidebar:
    ...
    spread_bps = st.slider("Спред (bps)", 10, 100, 35, 1)
    refresh_sec = st.slider("Интервал цикла (сек)", 5, 120, 15, 1)
    max_markets = st.slider("Макс. рынков", 1, 8, 2, 1)

    st.divider()
    st.subheader("Risk")
    max_cycle_notional = st.slider("Лимит notional на цикл (USDC)", 5, 500, 30, 1)
    max_portfolio_exposure = st.slider("Лимит экспозиции портфеля (USDC)", 5, 1000, 45, 1)
    max_token_position = st.slider("Лимит позиции на токен (shares)", 1.0, 200.0, 15.0, 1.0)
    sell_inventory_buffer = st.slider("Буфер для SELL (shares)", 0.0, 50.0, 1.0, 0.5)
    collateral_utilization = st.slider("Использование collateral", 0.1, 1.0, 0.6, 0.05)
    min_market_volume = st.slider("Мин. объем рынка 24h", 0, 500000, 5000, 1000)
